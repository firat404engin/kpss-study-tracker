...
try
{
    ...
    using (var pgConn = new NpgsqlConnection(postgresConn))
    {
        ...
        Console.WriteLine("🧹 Mevcut veriler temizleniyor...");
        
        await new NpgsqlCommand("TRUNCATE TABLE \"DenemeSonuclari\", \"Denemeler\", \"KullaniciKonuIlerlemeleri\", \"Konular\", \"Dersler\", \"Kullanicilar\" RESTART IDENTITY CASCADE", pgConn).ExecuteNonQueryAsync();
        Console.WriteLine("✓ Temizleme tamamlandı\n");
    }
    ...
    await MigrateTable(mssqlConn, postgresConn, "DenemeSonuclari", "Id", "MockExamId", "LessonId", "Correct", "Wrong");
